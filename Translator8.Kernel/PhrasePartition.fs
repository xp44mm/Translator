module Translator8.Kernel.PhrasePartition
...
let search (engs: string Set) (phrase: string) =
    ...
    engs |> Set.filter (fun eng -> rgx.IsMatch(eng))
        ...
        | head :: tail ->
            ...
            let rec shorten (pTokens: Token list, tokens: Token list) =
                ...
                | head :: tail ->
                    ...
                    let found =
                        engs
                        |> Set.exists (fun eng -> eng.Equals(phrase, StringComparison.OrdinalIgnoreCase))
                    //如果在词典中发现短语，就返回这个状态。
                    if found then
            ...
            let accTokens, tokens = lengthen engs [ head ] tail |> shorten
            accTokens, tokens
